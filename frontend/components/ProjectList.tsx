...
import type { ProjectLine } from "@/lib/snapshot";

interface Props {
  projects: ProjectLine[];
  liveStats?: Record<string, { open_prs: number; commits_7d: number; last_commit_at: string | null }>;
}

function Bullet({ color }: { color: ProjectLine["bullet"] }) {
  const base = "inline-block w-2 h-2 rounded-full";
  if (color === "red")  return <span className={`${base} bg-signal`} style={{ animation: "bullet-pulse 2.4s ease-in-out infinite" }} />;
...
export default function ProjectList({ projects, liveStats }: Props) {
  ...
  return (
    <section className="mt-16 pt-5 border-t border-ink">
      ...
      <div>
        {active.map((p) => <ProjectRow key={p.slug} project={p} live={liveStats?.[p.slug]} />)}
        {held.length > 0 && (
          <div className="my-4 relative h-px bg-ink">
            <span className="absolute left-1/2 -translate-x-1/2 -top-[7px] bg-paper px-3.5 font-mono text-[10px] uppercase tracking-[0.24em] text-ink font-semibold">HELD</span>
          </div>
        )}
        {held.map((p) => <ProjectRow key={p.slug} project={p} live={liveStats?.[p.slug]} dimmed />)}
      </div>
    </section>
  );
}

function ProjectRow({ project, live, dimmed }: { project: ProjectLine; live?: { open_prs: number; commits_7d: number; last_commit_at: string | null }; dimmed?: boolean }) {
  return (
    <Link
    ...
    >
      ...
      <span className="font-disp text-[17px] font-semibold tracking-[-0.01em] text-ink">{project.name}</span>
      {/* `kind` is a registry field; backend emission lands in Phase B. */}
      <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-mute">{project.kind ?? ""}</span>
