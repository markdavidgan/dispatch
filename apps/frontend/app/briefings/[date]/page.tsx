...
import { fetchBriefing } from "@/lib/briefings";
...
export default async function BriefingDetail({ params }: Props) {
  const { date } = await params;
  const brief = await fetchBriefing(date);
  if (!brief) notFound();

  // The project type from BriefingDetail uses `bullet: string`. Cast to the
  // shape ProjectList expects (its `bullet` is "red" | "amber" | "sand").
  const projects = brief.projects as Parameters<typeof ProjectList>[0]["projects"];

  const active = projects.filter((p) => p.status === "active").length;
  const held = projects.filter((p) => p.status === "held").length;
